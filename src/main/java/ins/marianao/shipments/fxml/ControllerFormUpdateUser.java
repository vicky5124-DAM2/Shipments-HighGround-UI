...
import cat.institutmarianao.shipmentsws.model.*;
...
public class ControllerFormUpdateUser implements Initializable, ChangeListener<Pair<String, String>> {
	...
	public void initialize(URL url, ResourceBundle resource) {
		...
		List<Pair<String,String>> roles = Stream.of(Role.values()).map(new Function<Role,Pair<String,String>>() {
			@Override
	...
	public void changed(ObservableValue<? extends Pair<String, String>> observable, Pair<String, String> oldValue, Pair<String, String> newValue) {
		if (observable.equals(this.cmbRole.valueProperty())) {
			if (Role.COURIER.name().equals(newValue.getKey())) {
				// Couriers
				this.enableCourierFields();
			} else {
	...
	@FXML
	public void updateProfileClick(ActionEvent event) {
		try {
			...
			if (user != null && !this.edicio) {
				...
				if (Role.COURIER.name().equals(role.getKey())) user = Courier.builder().username(strUsername).role(Role.COURIER).password(password).fullName(strFullName).extension(numExt).company(company).build();
				
				if (Role.RECEPTIONIST.name().equals(role.getKey())) user = Receptionist.builder().username(strUsername).role(Role.RECEPTIONIST).password(password).fullName(strFullName).extension(numExt).office(office).place(strPlace).build();
				
				if (Role.LOGISTICS_MANAGER.name().equals(role.getKey())) user = LogisticsManager.builder().username(strUsername).role(Role.RECEPTIONIST).password(password).fullName(strFullName).extension(numExt).office(office).place(strPlace).build();

				updateUserProfile(user, true);
			}
		} catch (Exception e) {
	...
	private void updateUserProfile(User user, boolean insert) throws Exception {
		final ServiceSaveUser updateUser = new ServiceSaveUser(user, insert);
